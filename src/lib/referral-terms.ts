 ...
 * 紹介料規約（暫定版）
 *
 * サイト内の既存記載（対象額（総支援金額−手数料−手数料に係る消費税）の2% / 募集終了時に明細発行 /
 * 終了月の翌々月末に指定口座へ入金）をベースに作成。
 * 確定版が用意され次第、この定数を差し替えれば
 * フォーム表示・確認メールの両方に反映される。
 */

export const REFERRAL_TERMS_VERSION = '1.2（暫定版）';
...
export const REFERRAL_TERMS_HISTORY: { version: string; date: string; summary: string }[] = [
  { version: '1.2（暫定版）', date: '2026-08-17', summary: '第3条の紹介料計算式を明確化（対象額＝総支援金額−手数料−手数料に係る消費税）・例示を更新' },
...
export const REFERRAL_TERMS: TermsArticle[] = [
  ...
  },
  {
    heading: '第3条（紹介料の額）',
    body: [
      '紹介料は、紹介者のクラウドファンディングプロジェクトにおける総支援金額から、当社所定の手数料および当該手数料に係る消費税を控除した額（以下「対象額」）の2%とします。',
      {
        text: '計算式は次のとおりとします。',
        items: [
          '対象額 ＝ 総支援金額 − 手数料 − 手数料に係る消費税',
          '紹介料 ＝ 対象額 × 2%',
        ],
      },
      {
        text: '例（手数料20%・消費税10%で試算した場合）',
        items: [
          '総支援金額100万円 → 対象額78万円 → 紹介料 約1.5万円（15,600円）',
          '総支援金額1,000万円 → 対象額780万円 → 紹介料 約15.6万円（156,000円）',
        ],
      },
      '上記の例は手数料20%・消費税10%で試算した参考値です。実際の手数料率により金額は変動します。',
    ],
  },
  {
