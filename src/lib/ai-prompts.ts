...
🔴 全体で**${key === 'closing' ? '350〜500' : '450〜700'}文字**。数字・固有名詞・具体的な場面を必ず入れ、抽象論だけで埋めないこと。${key === 'closing' ? `
   🔴 この「最後に」は**締めの章**なので、他の章より短くまとめてください。
   長く書こうとして途中で字数が尽き、**結びのお願いが書かれないまま終わるのが最悪**です。` : ''}
...
🔴 **最後の一覧項目「${outline?.slots?.[outline.slots.length - 1] ?? ''}」は、必ず最終段落として書き切ってください。**
   途中で字数が尽きて締めが消えるのが最悪です。**前半を短めに配分**し、
   最終段落まで必ず到達させてください。
