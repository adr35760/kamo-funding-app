...
const LONG_TEXT_OUTLINES: Record<string, { label: string; slots: string[] }> = {
  background: {
    ...
    slots: [
      ...
      'それでも足りなかった理由', '自己資金だけでは届かない事情', 'だから今回挑戦する',
    ],
  },
  vision: {
    label: 'ビジョン（実現したい未来）',
    slots: [
      ...
      'そこで生まれる新しい関係', 'それが続いた先にあるもの', 'その未来に向けた決意',
    ],
  },
  appeal: {
    ...
    slots: [
      ...
      '支援が何に変わるか', '一緒に実現したい未来', '少額でも力になる理由', '支援のお願い',
    ],
  },
  overview: {
    label: 'プロジェクト概要',
    slots: [
      ...
      'その課題の具体例', 'どうやって解決するか', '目標金額', '募集期間と資金の使い道の要点',
      'リターンの方針', '進捗の報告方針',
    ],
  },
  why_started: {
    ...
    slots: [
      ...
      'それでは足りない理由', '自分ひとりでは無理だと考えた点', 'クラウドファンディングを選んだ理由',
    ],
  },
  what_creates: {
    label: 'この企画で何を創出するのか',
    slots: [
      ...
      '続けることで積み上がるもの', '他の事業者に広がる可能性', 'その先の展望',
    ],
  },
};
...
【書き方（最重要）】
🔴 **下の①〜⑪を、1つにつき1文ずつ、全部書いてください。11文になります。**
   **①〜⑪の11個すべてを埋めることが必須**です。番号を飛ばしたら不合格です。
🔴 **1文は45〜55文字程度**で書いてください。**30字未満の短い文は不合格です。**
   各文に「理由」「具体例」「数字」「場面」のいずれかを足すと自然に45字を超えます。
   例（悪い＝28字）「設備の老朽化が進み修理費が増えています。」
   例（良い＝52字）「15年目の設備は昨年3回止まり、修理費は累計38万円に達し、経営を圧迫しています。」
🔴 **11文 × 45〜55字で、合計400〜440文字**に収めてください。
   **合計が440文字を超えないこと。** 超えるとシステム側で末尾が切られ、⑩⑪が消えます。
🔴 書き終えたら**自分で文の数と1文の長さを見直し、30字未満の文があれば具体例を足して
   45字以上に直してから出力してください。**
🔴 番号（①②…）や箇条書き記号は**本文に書かないこと**。番号は書く内容の順番を示すだけです。
   つなげて**地の文（散文）**にしてください。
🔴 **必ず句点（。）で言い切る**こと。「三つあります」と数を宣言したら、その数だけ列挙し切ること。
🔴 数字・固有名詞・具体的な場面を入れること。抽象論だけで11文埋めないでください。
