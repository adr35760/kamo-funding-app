...
🔴 **7つの項目すべてを、それぞれ「11文以上」で書いてください。** 10文以下は不合格です。
   ...
   **11文を書き切ることを守ってください。**
🔴 **ただし1項目は440文字を超えないこと。** 超えるとシステム側で末尾が切られ、
   ⑥⑦の内容が消えてしまいます。**①〜⑦を全部入れ切るために、1文を長くしすぎず
   40〜50字で簡潔にまとめてください。**
🔴 下の【各項目に含める内容】は①〜⑦の骨組みです。**①〜⑦は必ず全部書いたうえで、
   そのうちのいくつか（具体例・数字・場面の描写がある項目）を2文に分けて書き、
   合計11文以上にしてください。** 骨組みの順序は変えないでください。
🔴 各項目は**必ず句点（。）で言い切る**こと。「三つあります」と数を宣言したら、その数だけ列挙し切ること。
🔴 箇条書き・記号の羅列にしないこと。**地の文（散文）**で書いてください。
🔴 数字・固有名詞・具体的な場面を入れること。抽象論だけで11文埋めないでください。

【各項目に含める内容（①〜⑦を順に書き、いくつかを2文に割って合計11文以上にする）】
