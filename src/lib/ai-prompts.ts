...
【書き方（最重要）】
🔴 **下の①〜⑬を、1つにつき1文ずつ、全部書いてください。13文になります。**
   **①〜⑬の13個すべてを埋めることが必須**です。番号を飛ばしたら不合格です。
🔴 **1文は30〜35文字程度**で書いてください。13文 × 30〜35字で**約400〜440文字**です。
   各文に「理由」「具体例」「数字」「場面」のいずれかを入れてください。
   例（薄い＝20字）「修理費が増えています。」
   例（良い＝34字）「昨年は設備が3回止まり、修理費は累計38万円に達しました。」
🔴 **合計が440文字を超えないこと。** 超えるとシステム側で末尾が切られ、⑫⑬が消えます。
   13文すべてを入れ切るために、1文を35字より長くしないでください。
🔴 番号（①②…）や箇条書き記号は**本文に書かないこと**。番号は書く内容の順番を示すだけです。
   つなげて**地の文（散文）**にしてください。
🔴 **必ず句点（。）で言い切る**こと。「三つあります」と数を宣言したら、その数だけ列挙し切ること。
🔴 数字・固有名詞・具体的な場面を入れること。抽象論だけで13文埋めないでください。
