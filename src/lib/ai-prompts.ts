...

{
  "project": {
    ...
    "project_type": "実行確約型",
    "story": {
      "lead": "冒頭のリード文（読者を引き込む一文。**ここだけは短く**）",
      "background": "事業の背景と現状の課題（400文字）",
      "vision": "このプロジェクトが実現したい未来（400文字）",
      "use_of_funds": "資金の使途（具体的に・400文字）",
      "schedule": "実施スケジュール（日程の列挙。**字数を稼がない**）",
      "appeal": "支援者へのメッセージ・訴求（400文字）"
    },
    "creator": {
...
【story（掲載本文）の文字数要件（必須）】
🔴 **background / vision / use_of_funds / appeal の4項目は、各400文字以上**（360文字未満は不可・上限440文字）で書いてください。
   🔴 **文字数を「数える」のではなく、文の本数で書いてください。各項目とも必ず「7文以上」**書くこと。
      6文以下は不合格です。日本語の1文は平均55〜65文字なので、7文書けば自然に400文字を超えます。
      「400文字ぴったりを狙って数える」ことはしないでください。**7文書き切ることだけを守れば足ります。**
   各項目には次の内容を必ず含めてください（これを入れれば7文になります）:
   - background: ①事業の現状 ②現状の数字（売上構成・客数・季節変動など） ③課題が起きている具体的な場面
     ④その課題を放置した場合のリスク ⑤これまで試した対策 ⑥それでも足りなかった理由 ⑦だから今回挑戦する、の7文
   - vision: ①実現したい状態 ②支援者にとっての変化 ③地域にとっての変化 ④事業者にとっての変化
     ⑤1年後の姿 ⑥3年後の姿 ⑦その未来に向けた決意、の7文
   - use_of_funds: ①資金の総額と使途の全体像 ②〜⑥各費目の金額とその費目が必要な理由（5費目ぶん）
     ⑦使途と進捗の報告方法、の7文
   - appeal: ①読んでくれたお礼 ②起案者の実体験のエピソード ③そこで感じたこと ④このプロジェクトへの想い
     ⑤支援者が受け取るもの ⑥一緒に実現したい未来 ⑦支援のお願い、の7文
   4項目とも**必ず文を言い切り、句点（。）で終える**こと。「三つの理由です」のように数を宣言したら、その数だけ必ず列挙し切ること。
🔴 **lead は400文字にしない**。掲載ページの冒頭で読者を引き込む**短いリード文**（2〜3文程度）にしてください。
   ここが長いと読者が冒頭で離脱します。字数を稼がないこと。
🔴 **schedule も400文字にしない**。募集期間・実行・リターン発送の**日程の列挙**として書き、
   水増しの説明文を混ぜないこと。日付が読み取りにくくなります。
...
🔴 2〜4は**書き終える前に必ず文字数を数え、400文字に届いていなければ具体例・数字・エピソードを足して書き足す**こと。短い要約で終わらせないでください。
🔴 **文字数を数えるのではなく、文の本数で書いてください。2〜4は各項目とも必ず「7文以上」**書くこと。
   6文以下は不合格です。日本語の1文は平均55〜65文字なので、7文書けば自然に400文字を超えます。
   - overview: ①誰が ②何を ③誰に向けて ④なぜ（課題） ⑤どうやって ⑥目標金額と期間 ⑦リターンと報告方針、の7文
   - why_started: ①きっかけの出来事 ②そのときの状況 ③感じた危機感 ④現状の課題 ⑤これまでの取り組み
     ⑥それでは足りない理由 ⑦クラウドファンディングを選んだ理由、の7文
   - what_creates: ①創出する価値の総論 ②支援者にとっての価値 ③地域にとっての価値 ④業界にとっての価値
     ⑤新しく生まれる仕組み ⑥続けることで積み上がるもの ⑦その先の展望、の7文
🔴 2〜4は**必ず文を言い切り、句点（。）で終える**こと。「三つの価値です」のように数を宣言したら、その数だけ必ず列挙し切ること。
