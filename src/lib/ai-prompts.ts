...
5. story は use_of_funds（資金用途）と schedule（スケジュール）だけを書く。lead / background / vision / appeal は空文字にする（t iku指示 2026-09-22 の掲載順から外れたため）
6. リターンは「商品 / サービス / 体験 / スポンサー」の4カテゴリで構成し、各カテゴリを必ず1件以上生成する
7. スポンサー枠は**応援 / パートナー / メイン**の3段階の名称を使用（t iku指示 2026-09-22。旧: 松竹梅、さらに旧: ブロンズ〜ダイヤモンド）
8. project.extended の各項目（名称案3案・概要・なぜ・創出・最後に・発表会企画・活動歴・費用内訳）を必ず生成する
9. 文字数指定は厳守する
10. extended.title_proposals と project.title は**各25文字以内**で、**必ず「〜したい！」で終える**。記号や空白で字数を稼がず、日本語として自然な名称にする。**字数を埋めるための詰め物は禁止**（短くてよい）
10-2. project.subtitle は**30文字以内**。タイトルを補って「何をするのか」が分かる一文にする。締めの決まりは無い
11. extended.cost_breakdown の amount の合計は goal_amount と完全に一致させる
12. 🔴 **一人称は「私たち」で全文を統一する**（t iku指示 2026-09-24）。同じ文章の中で「私たち」と起案者名（「生島正は〜」）が混ざると、誰が語っているのか読み手が分からなくなる。
    - 地の文の主語は**「私たち」**にする
    - 起案者名は**プロフィール的に一度触れる程度**にとどめ、地の文の主語として繰り返さない
    - 「弊社」「当社」「私」は使わない（個人・法人のどちらでも「私たち」で通す）`;
...

{
  "project": {
    ...
    "subtitle": "サブタイトル（30文字以内。タイトルを補って何をするのかが分かる一文）",
    "main_image_url": "",
    ...
    "extended": {
      "title_proposals": ["25文字以内・したい！で終わる名称案1", "25文字以内・したい！で終わる名称案2", "25文字以内・したい！で終わる名称案3"],
...
1. title_proposals: プロジェクト名称の提案を**3案**。**各案は25文字以内**（日本語の文字数。半角空白・記号での字数稼ぎは禁止）で、**必ず「〜したい！」で終える**（例:「地元食材のデリバリーを地域のみんなと実現したい！」）。3案はそれぞれ切り口を変える（例: 価値訴求型／課題解決型／共感喚起型）
   🔴 **上限であって、ちょうど25文字にする必要はありません。** 20文字で言い切れるなら20文字で構いません。**字数を合わせるために意味のない語を足すことを固く禁じます**（「計画を」「をみんなの力で」等）。
   🔴 **project.title も同じルール（25文字以内・「〜したい！」締め）**で生成すること。名称案と主タイトルで規則が違うと掲載時に不整合になる。
...
🔴 **一人称は「私たち」で統一してください。** 同じ文章の中で「私たち」と起案者名
   （「${input.creatorName}は〜」）が混ざると、誰が語っているのか分からなくなります。
   **地の文の主語は必ず「私たち」**にし、起案者名を主語として繰り返さないこと。
   「弊社」「当社」「私」も使いません。
...
🔴 **段落に分けてください。** 1段落1〜3文、段落のあいだは空行（改行2つ）。
...
🔴 **一人称は「私たち」で統一**してください。起案者名を地の文の主語として繰り返さないこと。
🔴 **段落は5〜8段落**に保ってください（1段落1〜3文）。
