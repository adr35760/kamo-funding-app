  ...
  try {
    const response = await fetch(`${args.apiBaseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            // 🔴 2026-09-24 全面改訂。ここは**旧仕様のまま取り残されていた**:
            //   「①〜⑪のとおり1スロット1文で合計11文」「箇条書きは禁止」と書いてあり、
            //   いまのユーザープロンプト（段落構成・箇条書きを必ず使う・素材から3〜5個選ぶ）と
            //   真っ向から矛盾していた。system の指示が効いて箇条書きが出ない項目があった原因。
            role: 'system',
            content: [
              'あなたは日本語のクラウドファンディングページのライターです。',
              '指定された1項目を、掲載ページにそのまま貼れる読み物として書いてください。',
              '段落に分け、段落のあいだは空行で区切ります。並列する要素は「・」の箇条書きにします。',
              '🔴 一人称は「私たち」で統一してください。起案者名を地の文の主語として繰り返さないでください。',
              '「弊社」「当社」「私」は使いません。',
            ].join(''),
          },
        ...
        temperature: 0.7,
        // 1項目ぶん（約300〜450字）。JSONは1キーだけなので構造ぶんの余裕は要らない。
